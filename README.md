...
**Neither.** Every feedback report — rich, sparse, or absent — leaves the score
flat (0 of 9 runs reach the zone). The model knows the right change (picks the
rent-reduced board 24 of 24 times) yet repairs a bad board in 0 of 5 attempts;
the failure survives stripping the game away entirely, and its committed edit
is worse than 92% of its own samples. The bottleneck is
**constructing the edit**. When the model only *names* the knob and direction
and the harness sizes, applies, and stops the change, the same model succeeds
**9 of 9 in a median of two edits** — a competent chooser, not a competent
constructor.

See the project report (`report.pdf`) for the full write-up — diagnosis →
repair, eight findings.
